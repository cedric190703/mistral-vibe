...
// Console capture runs in the page's MAIN world (where the page's own scripts
// log), installing a hook that buffers messages so a later `console` action can
// read them. Idempotent, so it is safe to call before every action.
async function ensureConsoleHook(tabId) {
  try {
    ...
    // page may forbid injection (chrome://); ignore
  }
}

async function readConsole(tabId, limit) {
  try {
    const [res] = await chrome.scripting.executeScript({
      target: { tabId },
      world: "MAIN",
      func: (n) => (window.__vibeConsole || []).slice(-n),
      args: [limit],
    });
    return res.result || [];
  } catch {
...
async function handle(msg) {
  ...
  if (msg.action === "console") {
    const tab = await activeTab();
    await ensureConsoleHook(tab.id);
    return { console: await readConsole(tab.id, msg.maxConsole ?? 50) };
  }

  const tab = await activeTab();
  // Install the console hook before acting so this action's own logs are caught.
  await ensureConsoleHook(tab.id);
  switch (msg.action) {
    case "navigate":
    ...
    case "type":
      ...
      break;
    case "press_key":
      await exec(tab.id, pressKeyInPage, [msg.key]);
      break;
    case "scroll":
...
// Runs in the page's MAIN world: buffers console output + uncaught errors so the
// `console` action can report them. Installed once per page (idempotent).
function installConsoleHook() {
  if (window.__vibeHooked) return;
  window.__vibeHooked = true;
  window.__vibeConsole = [];
  const push = (type, args) => {
    try {
      const msg = args
        .map((a) => (typeof a === "string" ? a : JSON.stringify(a)))
        .join(" ");
      window.__vibeConsole.push("[" + type + "] " + msg);
      const over = window.__vibeConsole.length - 300;
      if (over > 0) window.__vibeConsole.splice(0, over);
    } catch (e) {}
  };
  for (const t of ["log", "info", "warn", "error"]) {
