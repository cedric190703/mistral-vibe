...
- `vibe-in-chrome` Chrome extension (`distribution/chrome-extension/`): drive the user's own browser — real profile, logins, and tabs — over a local WebSocket, with automatic fallback to Playwright when no extension is connected (`prefer_extension`). Adds tab management (`list_tabs`, `open_tab`, `switch_tab`), and every action (including `console` and `screenshot`) runs against the real tab. Snapshots now include disabled controls (flagged) so the agent sees locked buttons instead of treating them as missing.
