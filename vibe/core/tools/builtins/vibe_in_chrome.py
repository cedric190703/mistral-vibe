...
# Every action can run against the user's real browser through the extension.
# `pause` hands off to the human host-side first, then reads the page like a
# snapshot, so it never falls back to a separately launched Playwright browser.
...
class VibeInChrome(
    ...
    def _build_extension_payload(self, args: VibeInChromeArgs) -> dict[str, Any]:
        # `pause` has already handed off to the human host-side; on the wire it is
        # just a fresh read of the current page.
        wire_action = "snapshot" if args.action == "pause" else args.action
        payload: dict[str, Any] = {
            "action": wire_action,
            "maxElements": self.config.max_elements,
            "maxTextChars": self.config.max_text_chars,
            "maxConsole": self.config.max_console_messages,
        }
    ...
    async def _try_extension(
        ...
        result = VibeInChromeResult(
            ...
            tabs=tabs,
            text=text,
            console=[str(c) for c in (data.get("console") or [])],
        )
    ...
    async def run(
        ...
        # `pause` hands off to the human host-side, then re-reads the page on the
        # active backend (extension or Playwright) — so it never spawns a browser
        # of its own when the extension is driving the real Chrome.
        ...
        if args.action in _TAB_ACTIONS:
            ...
            await self._perform(_manager().page, args)
    ...
    async def _perform(self, page: Any, args: VibeInChromeArgs) -> None:
        ...
        match args.action:
            ...
            case "snapshot" | "console" | "screenshot" | "pause":
                pass
